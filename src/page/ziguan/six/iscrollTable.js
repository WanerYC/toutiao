...
export function createIScroller(selector) {
  ...
  function updatePosition() {
    ...
    let frozenfixed = document.querySelectorAll(selector + ' table tr th.fixedth');
    console.log(this)
    ...
    for (let i = 0; i < frozenfixed.length; i++) {
      frozenfixed[i].style.transform = 'translate(0px, ' + -1 * this.distX + 'px) translateZ(0px)';
    }
  }
...
export function refreshScroller() {
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < frozenfixed.length; i++) {
      frozenfixed[i].style.transform = 'translate(0px, 0px) translateZ(0px)';
    }
  }, 0);
}
